...
/// and indicates the procedure the system should follow
/// when determining whether or not to grant the permission to an application requesting it.
///
/// [Learn more](https://developer.android.com/guide/topics/manifest/permission-element#plevel)
public enum ProtectionLevel: String {
	/// **The default value.**
	///
	...
	/// and require confirmation before proceeding, or some other approach may be taken to avoid the user automatically allowing the use of such facilities.
	case dangerous
	
	/// A permission that the system grants only if the requesting application is signed with the same certificate
	/// as the application that declared the permission. If the certificates match, the system automatically grants
	/// the permission without notifying the user or asking for the user's explicit approval.
	case signature
	
	/// Old synonym for "signature|privileged" which is deprecated in API level 23.
	///
	/// A permission that the system grants only to applications that are in a dedicated folder
	/// on the Android system image or that are signed with the same certificate as the application
	/// that declared the permission. Avoid using this option, as the signature protection level
	/// should be sufficient for most needs and works regardless of exactly where apps are installed.
	///
	/// The **signatureOrSystem** permission is used for certain special situations where multiple vendors
